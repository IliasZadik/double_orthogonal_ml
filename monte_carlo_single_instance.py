...
def main(args):
    ...
    def plot_estimates(estimate_list, true_tau, title="Histogram of estimates"):
        ...
        plt.plot([tau, tau], [0, np.max(y)], 'b--', label='true effect')
    ...
    plt.subplot(1, 4, 1)
    bias_ortho, sigma_ortho = plot_estimates(np.array(ortho_rec_tau)[:,0].flatten(), tau, title="Orthogonal estimates")
    plt.subplot(1, 4, 2)
    plot_estimates(np.array(ortho_rec_tau)[:,1].flatten(), tau, title="Second order orthogonal")
    plt.subplot(1, 4, 3)
    plot_estimates(np.array(ortho_rec_tau)[:,2].flatten(), tau, title="Second order orthogonal with estimates")
    plt.subplot(1, 4, 4)
    bias_second, sigma_second = plot_estimates(np.array(ortho_rec_tau)[:,3].flatten(), tau, title="Second order orthogonal with estimates on third sample")
    plt.tight_layout()
    plt.savefig(os.path.join(opts.output_dir, 'recovered_coefficients_from_each_method_n_samples_{}_n_dim_{}_n_exp_{}_support_{}_sigma_q_{}.png'.format(n_samples,n_dim,n_experiments,support_size,sigma_q)), dpi=300, bbox_inches='tight')    
    plt.savefig(os.path.join(opts.output_dir, 'recovered_coefficients_from_each_method_n_samples_{}_n_dim_{}_n_exp_{}_support_{}_sigma_q_{}.pdf'.format(n_samples,n_dim,n_experiments,support_size,sigma_q)), dpi=300, bbox_inches='tight')
    ...
    plt.savefig(os.path.join(opts.output_dir, 'model_errors_n_samples_{}_n_dim_{}_n_exp_{}_support_{}_sigma_q_{}.png'.format(n_samples,n_dim,n_experiments,support_size,sigma_q)), dpi=300, bbox_inches='tight')    
    plt.savefig(os.path.join(opts.output_dir, 'model_errors_n_samples_{}_n_dim_{}_n_exp_{}_support_{}_sigma_q_{}.pdf'.format(n_samples,n_dim,n_experiments,support_size,sigma_q)), dpi=300, bbox_inches='tight')


    joblib.dump(ortho_rec_tau, os.path.join(opts.output_dir,'recovered_coefficients_from_each_method_n_samples_{}_n_dim_{}_n_exp_{}_support_{}_sigma_q_{}'.format(n_samples,n_dim,n_experiments,support_size,sigma_q)))
    joblib.dump(first_stage_mse, os.path.join(opts.output_dir,'model_errors_n_samples_{}_n_dim_{}_n_exp_{}_support_{}_sigma_q_{}'.format(n_samples,n_dim,n_experiments,support_size,sigma_q)))
